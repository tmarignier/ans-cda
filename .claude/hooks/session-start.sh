#!/usr/bin/env bash
# Prépare l'environnement Claude Code (web) : SDK .NET 8 + restauration NuGet de la solution.
set -euo pipefail
...
if ! command -v dotnet >/dev/null 2>&1; then
  ...
  apt-get install -y -qq dotnet-sdk-8.0 >/dev/null 2>&1 \
    || { apt-get update -qq >/dev/null 2>&1 && apt-get install -y -qq dotnet-sdk-8.0 >/dev/null 2>&1; }
fi
